bl_info = {
    ...
    "description": "Export scene with Transform Apply & Mesh Validation",
...
class AuroraExporter:
    ...
    def is_valid_mesh(self, obj):
        """[추가됨] 엔진 로드 에러 방지: 데이터가 없는 메쉬인지 확인"""
        if obj.type != 'MESH':
            return False
    ...
    def apply_object_transforms(self, obj):
        """[추가됨] 오브젝트의 트랜스폼을 메쉬 데이터에 고정 (Freeze Transforms)"""
        prev_active = self.context.view_layer.objects.active
        self.context.view_layer.objects.active = obj
        obj.select_set(True)
        
        # 위치, 회전, 스케일 적용 (Ctrl+A 효과)
...
class AURORA_PT_Panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        box.prop(props, "apply_transform", text="Apply Transforms (Freeze)")
        
        layout.separator()
        layout.operator("aurora.bake_scene", icon='EXPORT', text="Bake Aurora Map")
...
class AuroraProperties(bpy.types.PropertyGroup):
    ...
    apply_transform: bpy.props.BoolProperty(
        ...
        description="내보내기 전 모든 트랜스폼을 메쉬 데이터에 적용합니다 (권장).",
        default=True
    )

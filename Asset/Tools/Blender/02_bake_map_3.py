...
def convert_transform_aurora(obj):
    ...
    *** 중요: 엔진 포맷에 맞춰 Position/Scale에 4번째 값(padding) 추가 ***
    """
...
def create_model_component(obj):
    ...
    #            # Emission
    #            if 'Emission Color' in bsdf.inputs: # 4.0 이상 버전 호환성 체크 필요
    #                e = bsdf.inputs['Emission Color'].default_value
    #                strength = bsdf.inputs['Emission Strength'].default_value if 'Emission Strength' in bsdf.inputs else 1.0
    #                emission = [e[0]*strength, e[1]*strength, e[2]*strength, 1.0]
    #            elif 'Emission' in bsdf.inputs: # 구버전
    #                 e = bsdf.inputs['Emission'].default_value
    #                 emission = [e[0], e[1], e[2], 1.0]

...
constexpr size_t ChannelCount = 64; //profiling ?ïÑ?öî
...
void SoundManager::CreateNodeData(const std::string& filename)
{
	...
	// ?äú?ãù ?åå?ùºÎØ∏ÌÑ∞
	float deltaMultiplier = 1.1f;   // ?Ç• ÎØºÍ∞ê?èÑ
	float decayRatio = 0.3f;        // ?Ç• Í∏∏Ïù¥ (30%ÍπåÏ?? Í∞êÏá†)
...
void SoundManager::BGM_Shot(const std::string filename)
{
	...
	if (it != BGM_List.end())
	{
		...
		// Ïß??ó∞ ?ãúÍ∞? (Ï¥? ?Üí ?Éò?îå)
		float delaySec = 2.05f; // 2Ï¥? ?í§ ?û¨?Éù
